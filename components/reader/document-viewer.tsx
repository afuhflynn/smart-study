...
import { ChevronLeft, ChevronRight, Type, Bookmark, BookmarkCheck } from 'lucide-react';
...
export function DocumentViewer({ 
  ...
  const getCurrentChapterContent = () => {
    ...
    const endIndex = nextChapter ? nextChapter.startIndex : document.content.length;
  ...
  const formatContentWithHighlight = (content: string) => {
    ...
    return content
      ...
      .map((line, lineIndex) => {
        if (line.startsWith('# ')) {
          return `<h1 class="text-3xl font-bold mb-6 text-gray-900 dark:text-white leading-tight">${line.slice(2)}</h1>`;
        } else if (line.startsWith('## ')) {
          return `<h2 class="text-2xl font-semibold mb-4 text-gray-800 dark:text-gray-100 leading-tight">${line.slice(3)}</h2>`;
        } else if (line.startsWith('### ')) {
          return `<h3 class="text-xl font-medium mb-3 text-gray-700 dark:text-gray-200 leading-tight">${line.slice(4)}</h3>`;
        } else if (line.startsWith('- ')) {
          return `<li class="ml-6 mb-2 text-gray-600 dark:text-gray-300 leading-relaxed list-disc">${line.slice(2)}</li>`;
        } else if (line.trim() === '') {
          ...
          return `<p class="mb-4 text-gray-600 dark:text-gray-300 leading-relaxed"><strong class="font-semibold text-gray-800 dark:text-gray-200">${line.slice(2, -2)}</strong></p>`;
        } else {
          // For regular paragraphs, handle word highlighting
          if (line.trim()) {
            const words = line.split(/(\s+)/);
            
            const highlightedLine = words.map((segment) => {
  ...
  const navigateChapter = (direction: 'prev' | 'next') => {
    ...
    } else if (direction === 'next' && currentChapter < document.chapters.length - 1) {
      onChapterChange(currentChapter + 1);
    }
  };
  ...
  useEffect(() => {
    if (highlightedWordIndex >= 0) {
      const highlightedElement = contentRef.current?.querySelector('.bg-yellow-200, .dark\\:bg-yellow-800');
      ...
      >
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            {document.chapters.length} chapters • {document.estimatedReadTime} min read
          </p>
        </div>
          ...
          {document.chapters.map((chapter, index) => (
            <motion.button
              ...
              className={`w-full text-left p-4 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-200 ${
                currentChapter === index ? 'bg-purple-50 dark:bg-purple-900/20 border-r-2 border-purple-600' : ''
              }`}
            >
              <div className="flex items-center justify-between">
                <span className={`text-sm font-medium ${
                  currentChapter === index ? 'text-purple-600 dark:text-purple-400' : 'text-gray-700 dark:text-gray-300'
                }`}>
                  {chapter.title}
                </span>
